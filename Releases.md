...
- The log line carries the id the platform gave the interaction — Telegram's `update_id`, Discord's message id — right after who and where: `search   · tg/text   · alex@private · id 592011 · q="чайка" · MISS · 1 found · db 26ms`. Two lines sharing an id are one interaction logged twice; two lines with different ids are two deliveries.
- The log line now times the whole request, not just its lookup: the cache probe (recorded whether it hits or misses — a miss pays for it too, and hiding it made the fallback look cheaper than it is), the image download and webp conversion (`conv`), and the send (`send`, on cached and fresh answers alike, so a HIT and a MISS can be compared). Timings print in the order a request goes through them: `cache 0.6ms db 9.0ms conv 840ms send 310ms`. On Discord the probe's existence check is inside the measurement, so both of its round trips are counted.
...
- Telegram search answers are cached whole, the way Discord's already were. A repeat query is replayed straight from Redis — no DB lookup, no image download and no upload — and is logged as a `HIT`. Previously only the per-image `file_id` was cached, so every repeat still ran the search and re-sent each card; the `file_id` cache is an upload cache, not a search cache. The key covers the language and the chat's attachment limit, so a group answer is never served into a private chat and vice versa. An answer is stored only once Telegram has confirmed every image, so a failed or partial send is never cached, and a stale `file_id` drops the key and rebuilds the answer instead of leaving the query broken until it expires.
