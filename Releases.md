...
- A slash command run in a channel where the bot can't post no longer fails with a generic "Oops..." message. The command itself succeeds, but delivering the result via a public channel send raised `DiscordAPIError[50001] Missing Access` (the bot lacks "Send Messages" there, or was user-installed in a server it isn't a member of). Both the `/deck` modal path and the direct-command path now detect the 50001 and reply privately with an actionable note asking the user to grant the permission or run the command in a channel where the bot can post.
